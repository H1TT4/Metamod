...
var ms_classes = {
    ...
    spectators: {
        ...
        g_paint_handler: function() {
            ...
            var spectators = self.get_spectators()
            var font = fonts().verdana
            ...
            for (var i = 0; i < 64; i++) {
                unsorted[i] = [i, false, "none"]
            }
            for (var i = 0; i < spectators.length; i++) {
                unsorted[i] = [i, true, "none"]
            }

            if (UI.IsMenuOpen() || spectators.length > 0) {
                if (self.spalpha <= 1) {
            ...
            for (var i = 0; i < spectators.length; i++) {
                if (Render.TextSizeCustom(spectators[i], font)[0] > self.latest_item_width) {
                    self.latest_item_width = Render.TextSizeCustom(spectators[i], font)[0]
            ...
            for (var i = 0; i < unsorted.length; i++) {
                ...
                Render[menu.GetValue("Enable text shadow") ? "ShadowStringCustom" : "StringCustom"](x + 7, sy, 0, spectators[i], [255, 255, 255, self.m_alpha[i] * 255], font)
                if (menu.GetValue("Enable 'Avatars'")) {
                    Render.FilledRect(x + self.width - 3 - 12 - 5, sy, 12, 12, [23, 23, 23, self.m_alpha[i] * 255])
                    Render.StringCustom(x + self.width - 3 - 12 - 1, sy + 1, 0, "?", [255, 255, 255, self.m_alpha[i] * 255], font1)
                }
    ...
    menu_set: {
        g_paint_handler: function() {
            ...
            if (palette != 0) {
                menu.SetVisible("Global color", false)
                menu.SetVisible("Fade offset", true)
                menu.SetVisible("Fade frequency", true)
                menu.SetVisible("Fade split ratio", true)
                menu.SetVisible("  ", true)
            } else {
                menu.SetVisible("  ", false)
                menu.SetVisible("Global color", true)
                menu.SetVisible("Fade offset", false)
                menu.SetVisible("Fade frequency", false)
                menu.SetVisible("Fade split ratio", false)     
            }

            if (menu.DropdownValue(elements, 0)) {
                menu.SetVisible(" ", true)
                menu.SetVisible("Watermark name", true)
                menu.SetVisible("Watermark suffix", true)
                menu.SetVisible("Watermark prefix", true)
            } else {
                menu.SetVisible(" ", false)
                menu.SetVisible("Watermark name", false)
                menu.SetVisible("Watermark suffix", false)
                menu.SetVisible("Watermark prefix", false)
            }

            if (menu.DropdownValue(elements, 1)) {
                menu.SetVisible("HZ", true)
            } else {
                menu.SetVisible("HZ", false)
            }

            if (menu.DropdownValue(elements, 4)) {
                menu.SetVisible("Enable 'Avatars'", true)
            } else {
                menu.SetVisible("Enable 'Avatars'", false)
            }
        }
    }
}
